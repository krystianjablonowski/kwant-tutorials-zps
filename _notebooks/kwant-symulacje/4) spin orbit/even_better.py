...
def make_system(params=dict(e_z=0., alpha=0,t=0.),L=51,W=51,turnoff=0,B_lead=True):
...
def main(params,L,W,turnoff):
    syst = make_system(L=L,W=W,turnoff=turnoff)
    ...
    plot_bands(W,params)
    E1=np.linspace(-1,-0.85,50)
    E2=np.linspace(-0.85,-0.3,600)
    E3=np.linspace(-0.3,1.2,400)
    E=np.concatenate((E1,E2,E3))
    ed=plot_conductance(syst, energies=E,params=params)
    
    E_M=[-0.5,0.1,0.3,0.5,0.8]
    #params["alpha"]=1.2
    plot_conductance(syst, energies=E,params=params)
    for i in E_M:
       plot_density(syst,i,params,it=0)
    return ed


main(params,L,W,turnoff)